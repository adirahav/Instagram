...
async function query(username) {
    ...
    posts = await posts.filter(post => post.createdBy._id ==  user._id)
    posts = await storageService.sort(posts, 'createdAt', 'desc')
    
    const tempRandomBoolen = Math.random() > 0.5
    const isFollowing = username !== loggedinUser.username && tempRandomBoolen
    const canFollow = username !== loggedinUser.username && !tempRandomBoolen
    
    return {
        user: user,
        info: {
            postsCount: posts.length,
            followersCount: Math.floor(Math.random() * 10001),
            followingCount: Math.floor(Math.random() * 10001),
            canEdit: username === loggedinUser.username,
